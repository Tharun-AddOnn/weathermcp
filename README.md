...
- **A form rendered *inline* in Claude.ai is not currently achievable — tested, not assumed.**
  The mechanism is [MCP Apps](https://claude.com/blog/interactive-tools-in-claude) (GA 2026-01-26),
  and this server implements it: `ui://weather/picker.html` with mime type
  `text/html;profile=mcp-app`, bound to `get_weather` through `_meta.ui.resourceUri`. Verified live
  on the Netlify deployment — `resources/list` and `tools/list` both carry the right shapes.
  **Claude.ai still renders only the fallback link, with no iframe** (checked 2026-09-11), matching
  [ext-apps#671](https://github.com/modelcontextprotocol/ext-apps/issues/671). Anthropic's rollout
  centres on directory connectors; custom ones appear not to be included yet.

  The code is left in place deliberately: it is inert on clients that ignore `_meta.ui`, and will
  light up on its own if support arrives, with no redeploy. See [src/apps/picker.ts](src/apps/picker.ts).
